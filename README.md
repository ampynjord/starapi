...
Starvis - Star Citizen Database & Toolset is an unofficial Star Citizen data wiki in active development: game data extraction, REST API, web interface, Discord bot, AI assistant, and database tooling.
...
- practical calculators sitting next to the data they use: combat, FPS, mining, crafting and trade;
...
| Directory | Role |
|---|---|
| `api/` | Express REST API, Swagger/OpenAPI, auth, admin and data routes. |
...
- `ihm/src/components/holo/` holds the shared CTM geometry cache used by the 3D holoviewer.
- `ihm/src/components/ship/ShipHoloViewer.tsx` is the ship-detail holoviewer.
...
| `db/prisma/schema/10-meta.prisma` | Users, roles, bug reports, changelog and extraction logs (corporation tables kept dormant after the wiki refocus). |
...
Traefik HTTPS
  -> IHM, Next.js
      -> /api/* route handlers for auth and admin browser flows
...
without rebuilding the extraction stack: Discord bots, overlays, companion apps, public tools, quality audits,
...
| Tools | `/compare`, `/ranking`, `/fps-calculator`, `/mining-calculator`, `/trade-calculator`, `/crafting-calculator` |
...
| Admin | `/admin`, `/admin/bug-reports`, `/admin/monitoring` |
| Legal | `/legal` |
...
| `admin` | Admin UI, user management, bug report moderation, monitoring and full operational access. |
